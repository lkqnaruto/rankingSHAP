...
class AttributionExplanation(Explanation):
    ...
    def safe_to_file(self, filename):
        print("Writing to ", filename)
        # Expecting self.explanation as list of (feature_word, shap_value)
        # self.explanation is a list of tuples (feature, shap_value), change the following code accordingly

        attribute_dataframe = pd.DataFrame(
            self.explanation, columns=["feature", str(self.query)]
        )
        if os.path.isfile(filename):
            attribute_dataframe = attribute_dataframe.merge(
                pd.read_csv(filename), how="outer", on="feature"
            )
        attribute_dataframe.to_csv(filename, index=False)
